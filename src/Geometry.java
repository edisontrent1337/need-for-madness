		...
		for (int j = this.t.nt - 1; j >= 0; --j) {
			...
			if (n4 > 2) {
				...
				do {
					...
					if (this.t.zy[j] != 0) {
						array2[n6] += (int) ((array3[n6] - (this.t.z[j] - this.m.z - this.t.radz[j])) * this.m.sin(this.t.zy[j]) / this.m.sin(90 - this.t.zy[j]) - this.t.radz[j] * this.m.sin(this.t.zy[j]) / this.m.sin(90 - this.t.zy[j]));
					}
					if (this.t.xy[j] != 0) {
						array2[n6] += (int) ((array[n6] - (this.t.x[j] - this.m.x - this.t.radx[j])) * this.m.sin(this.t.xy[j]) / this.m.sin(90 - this.t.xy[j]) - this.t.radx[j] * this.m.sin(this.t.xy[j]) / this.m.sin(90 - this.t.xy[j]));
					}
				} while (++n6 < 4);
